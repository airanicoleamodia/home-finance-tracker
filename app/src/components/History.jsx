...
export default function History({ cur, categories, members, refreshKey, onEdit }) {
  ...
  useEffect(() => {
    let on = true;
    Promise.all([api.getExpenses(monthKey), api.getIncome(monthKey)])
      .then(([ex, inc]) => {
        if (!on) return;
        const merged = [
  ...
  const shown = rows.filter((r) => filter === "all" || r._kind === filter);
      ...
      {shown.length === 0 ? (
        ...
        <div className="card list">
          {shown.map((r) => {
            ...
            const sub = `${d.getDate()} ${MONTHS[d.getMonth()].slice(0, 3)} · ${nameOf(r.paid_by)}${r.note ? " · " + r.note : ""}`;
            return (
              <button className="item" key={r.id} onClick={() => onEdit("expense", r)}>
                <div className="ic" style={{ background: hexA(c.color, 0.14) }}>{c.icon}</div>
                <div className="it-mid"><div className="t1">{c.name}</div><div className="t2">{sub}</div></div>
